...
Composition here is normative rather than epistemic, and the two shouldn't be run together. The question in this section is whether authority survives a chain of grants, so it's answered from \(R\) and the recorded transitions. A separate question asks whether warranted \emph{inferences} about a system compose, and the answer there is that adjacent supported projections license a joint conclusion only where their endpoints and assumptions align and dependence is carried through \citep{reynolds2026nonComposition}. The distinction matters because monotonic authority composition is a claim about a normative structure, not a licence to chain evidence about behaviour. A trajectory can be authorized at every step while the evidence that each step behaved as recorded fails to compose, which is a record-adequacy question and belongs to the evidentiary layer.
