...
Table~\ref{tab:delegation-failures} gives the diagnostic translation. The point is not to rename familiar attacks. The point is to ask what delegation-layer failure would have to be evidenced, monitored, or ruled out before the system's action could be treated as authorized.
...
The corresponding evaluation question is shown in Table~\ref{tab:delegation-evidence}. A delegation-assurance evaluation does not need every row for every deployment. The rows are evidence families. A calendar assistant, a coding agent, a medical summarization system, and an institutional triage tool will weight them differently. What should not disappear is the requirement that each safety claim name the evidence family it relies on.
