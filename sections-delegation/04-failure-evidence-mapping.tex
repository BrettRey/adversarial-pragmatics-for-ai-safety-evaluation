...
The corresponding evaluation requirements are shown in Table~\ref{tab:delegation-evidence}. The rows are candidate fields for an authorization or evaluation claim, not a universal checklist of the full institutional record. Evidentiary assurance asks what the preserved record warrants under a declared review procedure. Applicability and the dependency operator come from the prospectively versioned regime and action-class map. The framework assigns no weights and sums no rows into a scalar score. It applies noncompensation only where \(R\) or the validated assessment use makes the applicable requirements conjunctive; alternatives, discretion, and defeasible conditions retain their own structure.
...
Audit reconstruction & A third party can reconstruct the recorded authorization path, operating conditions, execution, and review path without relying on a model-generated statement that the action was authorized. & Trace-review tasks require reconstruction of governing sources, effective interval, standing by operation, purported transitions and assigned effects, joint approvals or guards, scope, configuration, proposal, gate decision, execution, and recorded policy reason. \\
	Automated-evaluator assessment & The evaluation record keeps the delegation dimensions separate and includes uncertainty rather than collapsing them into one dispositive label. & On held-out cases matched to a prospectively declared target, compare no-rubric, rubric-aided, and adversarially phrased conditions against independently justified regime-relative references. Interpretations and uses of the resulting labels require their own validation argument. \\
\bottomrule
\end{tabular}
...
Conversely, refusing a permitted comparison task can defeat task success without defeating action authorization. If \(R\) required completion and the omission is assessed, the refusal instead defeats a separately declared mandate-compliance claim. An inadequate trace creates a record gap at the affected claim node and may leave the declared use-level conclusion unevaluable; it doesn't turn missing evidence into evidence of unauthorized action. The target is the action-authorization claim, not the final answer alone.
