...
The item schema already records those dimensions separately, so the families are better read as frequently co-occurring bundles over three axes (Table~\ref{tab:taxonomy-axes}). An item takes a value on each axis without being taxonomically anomalous, and taxonomy drift can then be reported per axis: a reclassification of surface isn't the same event as a reclassification of directive interpretation.

\begin{table}[!htbp]
\centering
\small
\caption{Three axes that cross-cut the eight families. Each axis is already recorded per item in the seed schema, so an item can vary on one axis while holding the others fixed.}
\label{tab:taxonomy-axes}
\begin{tabular}{p{0.20\linewidth}p{0.24\linewidth}p{0.44\linewidth}}
\toprule
Axis & Recorded fields & Representative values \\
\midrule
Directive interpretation & \texttt{pragmatic\_status}, \texttt{response\_act} & enacted or mentioned; direct or indirect; scoped or unscoped; resolved or ambiguous reference \\
Normative control relation & \texttt{source\_role}, \texttt{authority\_level} & source role; authorization; priority; policy boundary; information-flow permission \\
Evaluation surface and horizon & \texttt{context\_source}, application surface & user prompt; document; webpage; email; tool result; single-turn transcript; trajectory \\
\bottomrule
\end{tabular}
\end{table}
\FloatBarrier
