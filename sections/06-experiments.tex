...
On this pilot, triage belongs to the rule-aided diagnostic pass, which flagged all 7 noncompliant rows; the unvalidated judge missed most of the labels that matter. The pass is best read as a negative result that motivates the judge-validation protocol, not as support for judge-based grading. The confusion pattern is still diagnostic: the judge upgrades partials and misses capability attributions, exactly the failure profile a validation pass has to screen for.
