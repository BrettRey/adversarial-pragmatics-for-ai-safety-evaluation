...
rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR/sections" "$OUT_DIR/figures" "$OUT_DIR/fonts"
...
do
  ...
  if [[ -z "$font_path" ]]; then
    echo "Missing font $font; cannot build self-contained arXiv bundle." >&2
    exit 1
  fi
  cp "$font_path" "$OUT_DIR/fonts/$font"
done
